import { BriefcaseBusiness, Globe, Linkedin, MailIcon, MapPin, Phone, UserIcon } from 'lucide-react'
  ...
  return (
    <div>
        ...
        <div className='flex items-center gap-2'>
            <label>
                {data.image ? (
                    <img src={typeof data.image === 'string' ? data.image : URL.createObjectURL(data.image)} alt="UserImage" className='w-16 h-16 rounded-full object-cover mt-6 ring-slate-300 hover:opacity-80' />
                ) : (
                    <div className='inline-flex items-center gap-2 mt-5 text-slate-600 hover:text-slate-700 cursor-pointer'>
                        <UserIcon className='size-10 p-3 border rounded-full '/>
                        Upload user image
                    </div>
                )}
            ...
            {typeof data.image === 'object' && (
                <div className='flex flex-col gap-1 pl-4 text-sm'>
                    ...
                    <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3' >
                        ...
                        <div className='w-9 h-5 bg-slate-300 rounded-full peer peer-checked:bg-indigo-400 transition-colors duration-200'>
